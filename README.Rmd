...
``` r
remotes::install_github("https://github.com/Landscape-Data-Commons/terradactyl")
```
...
`terradactyl` can be used to gather core rangeland monitoring data into a standard format, calculate standardized indicators, custom indicators, and produce consistent data models. There are also a few quality control scripts available to help users ensure that species data are handled correctly. The basic workflow of `terradactyl` is to first gather data into a standard format, also called a tall table. Then calculate indicators based on that format and grouping variables (if relevant). For some commonly calculated groups of indicators, such as for the AIM program or model inputs, there are also data model functions which take the tall table files and other data and produce data outputs specific to that user need.

Recommended Citation: McCord, S.E., J.R. Brehm, S.H. Burnett, C. Dietrich, B. Edwards, M. Hernandez-Narvaez, E. Kachergis, L.J. Metz, F. Pierson, K.S. Ramirez, N.G. Stauffer, N.P. Webb, C.E. Tweedie. A framework and toolset for standardizing ecosystem indicators. Ecological Indicators, vol 144, 2022. <https://doi.org/10.1016/j.ecolind.2022.109511>.

![Visualization of terradactyl workflow](C:/Users/jrbrehm/Documents/GitHub/terradactyl/man/figures/data-flow.png)
...
# Load the line-point intercept demo data.
demo_lpi_tall <- terradactyl::demo_data(type = "lpi")

# Calculate percent of each plot with bare soil.
# "Bare soil" corresponds to pin drops where no cover was recorded in canopy
# layers and the SoilSurface value in the code variable was "S".
# This can be calculated with pct_cover(hit = "first") and keeping only the
# cover for the variable S in the output, but it's easier to do with the wrapper
# function pct_bareground().
cover_baresoil <- terradactyl::pct_bareground(lpi_tall = demo_lpi_tall,
...
```{r  custom indicator}
# Add a variable called BigSagebrush that has the value "BigSage" for all
# records containing a qualifying species code in the variable called code and
# NA for all records that don't correspond to a big sagebrush species.

big_sagebrush_species <- c("ARTRW8", "ARTRT", "ARTRV", "ARTR2")
demo_lpi_tall <- dplyr::mutate(.data = demo_lpi_tall,
                               BigSagebrush = dplyr::case_when(code %in% big_sagebrush_species ~ "BigSage",
                                                               .default = NA))

# Use pct_cover() to calculate first hit cover using the variable BigSagebrush
# to determine the types of cover.
# Because the only values in BigSagebrush are NA and "BigSage", the resulting
# data frame will have a variable called BigSage with the percent cover
# provided by the combined species codes in big_sagebrush_species.
cover_artemisia <- pct_cover(lpi_tall = demo_lpi_tall,
                             hit = "first",
                             indicator_variables = "BigSagebrush")
...
Herrick et al. *Monitoring Manual for Grassland, Shrubland, and Savanna Ecosystems Volume 1: Core Methods, 2nd edition.* Jornada Experimental Range, Las Cruces New Mexico, 2017, page 30 <https://jornada.nmsu.edu/files/Core_Methods.pdf>

<!-- badges: start -->

[![R-CMD-check](https://github.com/Landscape-Data-Commons/terradactyl/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/Landscape-Data-Commons/terradactyl/actions/workflows/R-CMD-check.yaml) <!-- badges: end -->
