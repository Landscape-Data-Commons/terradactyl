...
aero <- function(lpi_tall,
  ...
  lapply(
    common_PK,
    function(X) {
      write.table(canopy_gap[canopy_gap$PrimaryKey == X, "Gap"],
  ...
  lapply(
    ...
    function(X) {
      cat(
        ...
        "[INPUT_VALUES]",
        paste("wind_location:",
          plots_texture$Latitude_NAD83[plots_texture$PrimaryKey == X] %>% unique(),
          plots_texture$Longitude_NAD83[plots_texture$PrimaryKey == X] %>% unique(),
          sep = " "
        ),
