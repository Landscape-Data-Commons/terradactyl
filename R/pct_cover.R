...
pct_cover <- function(lpi_tall,
  ...
  # if (verbose) {
  #   message(paste(indicator_variables,
  #                 collapse = ", "))
  # }
  ...
  if (length(hit) > 1) {
    stop(paste("hit must be a single character string. Valid values are 'any', 'first', or 'basal'."))
  }
  ...
  # which seems wrong? Surely this should happen as data cleaning before it
  ...
  if (length(indicator_variables) > 0) {
  ...
  ##### Removing records without indicator_variables values ----------------------
  ...
  indicators <- dplyr::filter(.data = indicators,
                              indicator %in% keeping_indicators)
  ...
  if (nrow(output) < 1) {
    warning("No data qualified for requested indicator(s). Returning NULL")
    return(NULL)
  }
