...
#' @param source "auto", "aim", or "lmf" (case independent). Read both AIM and LMF data, or force to only read one type.
#' @importFrom magrittr %>%
#' @name gather_all
#' @family <gather>
...
gather_all <- function(dsn = NULL, dflist = NULL, outfolder, outtype = c("csv", "rdata"),
                       verbose = T, doLPI = T, source = "auto") {
  # prep ####
  outtype <- tolower(outtype)
  source <- tolower(source)

  if(source == "aim") {
    print("Gathering AIM data only")
  } else if(source == "lmf"){
    print("Gathering LMF data only")
  } else {
    print("Gathering both AIM and LMF data")
  }
  ...
  # Gap ####
  if(source != "lmf" &
    (("tblGapDetail" %in% names_rda & "tblGapHeader" %in% names_rda) |
  ...
  if(source != "aim" & (("GINTERCEPT" %in% names_rda & "POINT" %in% names_rda) |
  ...
  # Soil stability ####
  if(source != "lmf" & (("tblSoilStabDetail" %in% names_rda & "tblSoilStabHeader" %in% names_rda) |
  ...
  if(source != "aim" & (("SOILDISAG" %in% names_rda) |
  ...
  # LPI ####
  if(doLPI == T){
    if(source != "lmf" & (("tblLPIDetail" %in% names_rda & "tblLPIHeader" %in% names_rda) |
      ...
      if(source != "aim" & (("PINTERCEPT" %in% names_rda) |
      ...
      if(source != "aim" & (("PASTUREHEIGHTS" %in% names_rda) |
  ...
  # Species inventory ####
  if(source != "lmf" & (("tblSpecRichDetail" %in% names_rda & "tblSpecRichHeader" %in% names_rda) |
  ...
  if(source != "aim" & (("PLANTCENSUS" %in% names_rda) |
  ...
  # soil horizons ####
  if(source != "lmf" & (("tblSoilPitHorizons" %in% names_rda) |
  ...
  if(source != "aim" & (("SOILHORIZON" %in% names_rda) |
  ...
  # soil summary ####
  if(source != "lmf" & (("tblSoilPitHorizons" %in% names_rda & "tblSoilPits" %in% names_rda) |
  ...
  # iirh ####
  if(source != "lmf" & (("tblQualDetail" %in% names_rda & "tblQualHeader" %in% names_rda) |
  ...
  if(source != "aim" & (("RANGEHEALTH" %in% names_rda) |
  ...
  if(source != "lmf" & (("tblPlots" %in% names_rda) |
    ...
    plotchar_aim <- NULL
    if(verbose) print("tblPlots not found. Skipping AIM Plot Characterization.")
  }
  if(source != "aim" & (("POINT" %in% names_rda & "POINTCOORDINATES" %in% names_rda & "GPS" %in% names_rda) |
     ("POINT" %in% names_gdb & "POINTCOORDINATES" %in% names_gdb & "GPS" %in% names_gdb))){
    if(verbose) print("Gathering LMF plot characterization")
    ...
    plotchar_lmf <- NULL
    if(verbose) print("POINT and/or GPS and/or POINTCOORDINATES not found. Skipping LMF Plot Characterization.")
  }
  ...
  names(list_out) <- c("Gap", "VegHeight", "SoilHorizons", "LPI", "SoilPitSummary",
    ...
    list_out <- list(
      gap_tall, hz_tall, pit_tall, plotchar_tall,
      soilstab_tall, spp_inventory_tall
    )
